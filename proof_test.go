...
type proofBenchmarkFixture struct {
	root              *Node
	rootHash          []byte
	singleProof       *Proof
...
func newProofBenchmarkFixture(b *testing.B) *proofBenchmarkFixture {
	b.Helper()
	...
	singleProof, err := root.Prove(leafCount + 513)
	if err != nil {
		b.Fatalf("failed to build benchmark proof: %v", err)
	}

	multiProof, err := root.ProveMulti([]int{leafCount + 513, leafCount + 514})
	if err != nil {
		b.Fatalf("failed to build benchmark multiproof: %v", err)
	}

	fullTreeIndices := make([]int, leafCount)
	for i := 0; i < leafCount; i++ {
		fullTreeIndices[i] = leafCount + i
	}

	adjacentProof, err := root.ProveMulti([]int{leafCount + 256, leafCount + 257})
	if err != nil {
		b.Fatalf("failed to build adjacent multiproof: %v", err)
	}

	fullTreeLeafProof, err := root.ProveMulti(fullTreeIndices)
	if err != nil {
		b.Fatalf("failed to build full tree multiproof: %v", err)
	}

	return &proofBenchmarkFixture{
		root:              root,
		rootHash:          root.Hash(),
...
// BenchmarkNodeProve measures the cost of building a single proof from a prebuilt tree.
func BenchmarkNodeProve(b *testing.B) {
	fixture := newProofBenchmarkFixture(b)

	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := fixture.root.Prove(fixture.singleProof.Index)
		if err != nil {
...
// BenchmarkNodeProveMulti measures the cost of building a small multiproof from a prebuilt tree.
func BenchmarkNodeProveMulti(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := fixture.root.ProveMulti(fixture.multiProof.Indices)
		if err != nil {
			b.Fatalf("failed to create multiproof: %v", err)
		}
	}
}

// BenchmarkVerifyProof measures the cost of verifying one single proof.
func BenchmarkVerifyProof(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		ok, err := VerifyProof(fixture.rootHash, fixture.singleProof)
		if err != nil {
...
// BenchmarkVerifyMultiproof measures small and dense multiproof verification workloads.
func BenchmarkVerifyMultiproof(b *testing.B) {
	fixture := newProofBenchmarkFixture(b)

	b.Run("adjacent_leaves", func(b *testing.B) {
		// This case measures a compact proof where neighboring leaves share most of the path.
		b.ReportAllocs()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			ok, err := VerifyMultiproof(
				fixture.rootHash,
				fixture.adjacentProof.Hashes,
				fixture.adjacentProof.Leaves,
				fixture.adjacentProof.Indices,
			)
			if err != nil {
				b.Fatalf("failed to verify multiproof: %v", err)
			}
			if !ok {
				b.Fatal("multiproof did not verify")
			}
		}
	})

	b.Run("all_leaves", func(b *testing.B) {
		// This case measures the dense path where the verifier rebuilds the whole tree from leaves.
		b.ReportAllocs()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			ok, err := VerifyMultiproof(
				fixture.rootHash,
				fixture.fullTreeLeafProof.Hashes,
				fixture.fullTreeLeafProof.Leaves,
				fixture.fullTreeLeafProof.Indices,
			)
			if err != nil {
				b.Fatalf("failed to verify multiproof: %v", err)
			}
